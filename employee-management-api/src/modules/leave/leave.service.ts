...
} from "./leave.reposistory";
import { ROLES, LEAVE_STATUS, LEAVE_ALLOWANCE_DAYS } from "../../constants/leave.constnt";
...
export const updateLeaveService = async (leaveId: string, employeeId: string, data: Partial<ILeave>) => {
...
export const cancelLeaveService = async (leaveId: string, employeeId: string) => {
    const leave = await getLeaveById(leaveId);
    if (!leave) {
        throw new AppError("Leave request not found", 404);
    }
...
export const getLeaveBalanceService = async (employeeId: string) => {
    ...
    const approvedDays = leaves
        .filter((leave: any) => leave.status === LEAVE_STATUS.APPROVED)
        .reduce((sum: any, leave: any) => sum + calculateLeaveDays(leave.startDate, leave.endDate), 0);
    const pendingDays = leaves
        .filter((leave: any) => leave.status === LEAVE_STATUS.PENDING)
        .reduce((sum: any, leave: any) => sum + calculateLeaveDays(leave.startDate, leave.endDate), 0);
    return {
...
export const getLeaveCalendarService = async (employeeId: string, role: string) => {
    const now = new Date();
